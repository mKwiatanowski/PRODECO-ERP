...
export const DocumentList: React.FC = () => {
    ...
    const fetchDocuments = async () => {
        try {
            setIsLoading(true);
            // @ts-ignore
            const data = await window.electron.getInventoryDocuments();
            setDocuments(data);
        } catch (error) {
    ...
    return (
        <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden flex flex-col h-full shadow-xl">
            {/* Header Toolbar */}
            <div className="p-6 border-b border-slate-700 bg-slate-800/80 flex items-center justify-between gap-4">
                ...
                <div className="relative w-full max-w-md">
                    ...
                    <input
                        type="text"
                        placeholder="Szukaj po numerze, typie lub referencji..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-10 pr-4 py-2.5 text-slate-300 focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all placeholder-slate-500"
                    />
                </div>
            </div>
                ...
                <table className="min-w-max w-full text-left border-collapse table-fixed">
                    <thead className="bg-[#161b22] sticky top-0 z-10 shadow-sm border-b border-slate-700">
                        <tr>
                            ...
                            <ResizableTh tableId="inventory_documents" columnId="type" initialWidth={120} className="px-6 py-4 font-semibold text-slate-400 uppercase text-xs tracking-wider border-b border-slate-700 text-center">Typ</ResizableTh>
